...
bool ZhijiaController::is_supported(CommandType cmd_type) {
    return (cmd_type == CommandType::CUSTOM) || (this->translate_cmd(cmd_type) != 0);
}

uint8_t ZhijiaController::translate_cmd(CommandType cmd_type) {
  uint8_t cmd = 0;
  switch(cmd_type)
  {
...
void ZhijiaController::get_adv_data(uint8_t * buf, Command &cmd) {
  if(cmd.type_ == CommandType::CUSTOM) {
    uint8_t args[] = {cmd.args_[1], cmd.args_[2], cmd.args_[3]};
    stage1(args, cmd.args_[0], cmd.tx_count_, GROUP, MAC, UUID1, buf);
  } else {
    uint8_t args[] = {cmd.args_[0], 0, 0};
    stage1(args, this->translate_cmd(cmd.cmd_), cmd.tx_count_, GROUP, MAC, UUID1, buf);
  }
  stage2(buf, 0x18, 0x2);
  stage3(buf, 0x1a, buf);
}
