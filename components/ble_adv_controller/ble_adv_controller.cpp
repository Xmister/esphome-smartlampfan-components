...
void BleAdvController::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Hash ID '%X'", this->get_object_id_hash());
...
void BleAdvController::enqueue(Command &cmd) {
  ...
  cmd.id_ = this->get_object_id_hash();
...
void BleAdvController::loop() {
  ...
  if(this->adv_start_time_ == 0) {
    // no on going command advertised, check if any to advertise
    if(!this->commands_.empty()) {
      ESP_LOGD(TAG, "BleAdvController::loop - start advertising at %d", now);
    ...
    // command is being advertised, check if stop and clean-up needed
    if (now > this->adv_start_time_ + this->tx_duration_) {
      ESP_LOGD(TAG, "BleAdvController::loop - stop advertising at %d", now);
