...
uint16_t sign(uint8_t* buf, uint8_t tx_count, uint8_t seed) {
...
bool FanLampController::is_supported(CommandType cmd_type) {
  return (cmd_type == CommandType::CUSTOM) || (this->translate_cmd(cmd_type) != 0);
}

uint8_t FanLampController::translate_cmd(CommandType cmd_type) {
  uint8_t cmd = 0;
  switch(cmd_type)
  {
...
void FanLampController::build_packet_v1(uint8_t* buf, Command &cmd) {
  ...
  packet->command = this->translate_cmd(cmd.cmd_);
...
void FanLampController::build_packet_v2(uint8_t * buf, Command &cmd, bool with_sign) {
  ...
  packet->command = translate_cmd(cmd.cmd_);
  packet->group_index = cmd.index_;
  packet->rand = seed;
  switch(cmd.cmd_)
  {
    case CommandType::CUSTOM:
      packet->command = cmd.args_[0];
      packet->args[0] = cmd.args_[1];
      packet->args[1] = cmd.args_[2];
      packet->args[2] = cmd.args_[3];
      packet->args[3] = cmd.args_[4];
      break;
    case CommandType::LIGHT_DIMCCT:
      if (this->reversed_) {
        packet->args[2] = cmd.args_[1];
        packet->args[3] = cmd.args_[0];
      } else {
        packet->args[2] = cmd.args_[0];
        packet->args[3] = cmd.args_[1];
      }
      break;
    case CommandType::FAN_SPEED:
      packet->args[2] = cmd.args_[0];
